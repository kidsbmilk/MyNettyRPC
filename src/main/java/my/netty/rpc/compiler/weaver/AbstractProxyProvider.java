...
public abstract class AbstractProxyProvider implements ClassProxy {
    ...
    public <T> T createProxy(ClassLoader classLoader, Object target, Interceptor interceptor, Class<?>... proxyClasses) { // 这个是接口ClassProxy里的另一个方法
        return createProxyProvider(classLoader, new InterceptorInvoker(target, interceptor), proxyClasses);
    }

    abstract <T> T createProxyProvider(ClassLoader classLoader, ObjectInvoker interceptorInvoker, Class<?>... proxyClasses); // 创建自己的抽象方法，使类继承以及接口实现层次更清晰。
}
